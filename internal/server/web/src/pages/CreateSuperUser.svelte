<script lang="ts">
  ...
  const createSuperUser = async () => {
    ...
    try {
      ...
      if (res.status == 200) {
        toast.success("Superuser created. Login to continue.");
  ...
  >
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
      <div class="bg-white py-8 px-4 border shadow sm:rounded-lg sm:px-10">
        <form class="space-y-6" on:submit|preventDefault={createSuperUser}>
          <div>
          ...
          <div>
            ...
            >
              ...
              {:else}
                <span class="my-auto">Create Superuser</span>
              {/if}
            </button>
          </div>
        </form>
